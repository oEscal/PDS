...
public class SoapGenerator {
    ...
    public static List<List<Character>> generate(List<String> words, int soap_size){
        ...
        createSoap( current_soap, soap_size);
        createPossiblePositions( possible_positions, soap_size);

        for (String word : words)
            insertRandom(current_soap, possible_positions, positions, word, possible_directions);
        putRandomCharsInSoap(current_soap);
    ...
    private static Integer[] generateRandomDirection(List<List<Character>> current_soap,
        ...
        while(current_possible_directions.size() > 0){
            ...
            for(index = 0; index < word.length(); index++){
                ...
                if((position_to_put[0] < 0 || position_to_put[1] < 0 || position_to_put[0] >= current_soap.size() || position_to_put[1] >= current_soap.size()) ||
                        (current_soap.get(position_to_put[0]).get(position_to_put[1]) != null &&
                                current_soap.get(position_to_put[0]).get(position_to_put[1]) != character_to_put))
                    break;
            }
    ...
    private static void insertWordInDirection(List<List<Character>> current_soap,
        ...
        for(int index = 0; index < word.length(); index++){
            ...
            current_soap.get(position_to_put[0]).set(position_to_put[1], character_to_put);
    ...
    private static void createSoap(List<List<Character>> current_soap, int soap_size){
        for ( int i = 0; i < soap_size; i++ ){
            current_soap.add(new ArrayList<>());
            for (int j = 0; j < soap_size; j++)
                current_soap.get(i).add(null);
        }
    }

    private static void createPossiblePositions(List<Integer[]> possible_positions, int soap_size){
    ...
    private static void putRandomCharsInSoap(List<List<Character>> current_soap){

        for(int row_index = 0; row_index < current_soap.size(); row_index++)
            for(int col_index = 0; col_index < current_soap.get(row_index).size(); col_index++)
                if(current_soap.get(row_index).get(col_index) == null)
                    current_soap.get(row_index).set(col_index, (char)('A' + rand.nextInt(26)));
    }
